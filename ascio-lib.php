...
Class ASCIO {
  ...
  public static function request($functionName, $ascioParams, $outputResult=false)  {	
  	...
  	if(is_array($result) && strpos($result["error"],"Invalid Session") > -1) {
  		SessionCache::clear();
  		return ASCIO::request($functionName, $ascioParams, $outputResult);		
  	} else {		
  ...
  private static function getDomain($handle) {
  	...
  	$result = ASCIO::request("GetDomain", $ascioParams,true); 
  	return $result;
  }
  ...
  private static function getOrder($orderId) {
  	...
  	$result = ASCIO::request("GetOrder", $ascioParams,true); 
  	return $result;
  }
