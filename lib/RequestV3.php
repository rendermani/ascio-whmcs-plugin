 ...
 *
 * This class provides the same public interface as Request.php (v2) but uses
 * the Ascio v3 Web Service API with SOAP header authentication.
 *
 * Key differences from v2:
 * - No session management needed (uses SOAP header authentication)
 * - Uses v3 service classes from ascio/ssl/v3/service/
...
/**
 * Domain cache for v3 API - caches domain objects to avoid repeated API calls
 */
Class DomainCacheV3 {
	public static function get($domainId) {
		global $ascioDomainCacheV3;
		if(!$ascioDomainCacheV3) $ascioDomainCacheV3 = array();
		return $ascioDomainCacheV3[$domainId] ?? null;
	}
	public static function put($domain) {
		...
		if ($domainId) {
			$ascioDomainCacheV3[$domainId] = $domain;
		}
	}
}
 ...
 * Implements the same public interface as the v2 Request class for compatibility
 */
Class RequestV3 {
	/** @var string */
	public $account;
	 ...
	 * Checks for v3 TLD plugins first, falls back to base RequestV3
	 *
	 * @param array $params WHMCS module params
	 * @return RequestV3
	 */
	public static function create($params) {
		...
		// Try v3-specific TLD handler first (in tld/v3/ subdirectory)
		if ($tld) {
			$v3Filename = $tldDir . "$tld/v3/$tld.php";
			if(file_exists($v3Filename)) {
				require_once($v3Filename);
				$className = "\\ascio\\v3\\domains\\" . str_replace(".", "_", $tld);
				if (class_exists($className)) {
					return new $className($params);
				}
			}

			// Try {tld}_v3.php naming convention
			$v3AltFilename = $tldDir . "$tld/{$tld}_v3.php";
			if(file_exists($v3AltFilename)) {
				require_once($v3AltFilename);
				$className = "\\ascio\\v3\\domains\\" . str_replace(".", "_", $tld);
				if (class_exists($className)) {
					return new $className($params);
				}
		...
		// Try parent TLD v3 handler (e.g., "ag.it" -> "it", "co.uk" -> "uk")
		$parentTld = self::getParentTld($tld);
		if($parentTld) {
			$parentV3Filename = $tldDir . "$parentTld/v3/$parentTld.php";
			if(file_exists($parentV3Filename)) {
				require_once($parentV3Filename);
				$className = "\\ascio\\v3\\domains\\" . str_replace(".", "_", $parentTld);
				if (class_exists($className)) {
					return new $className($params);
				}
			}
	...
	public function searchDomain() {
		...
		$domain = DomainCacheV3::get($domainId);
